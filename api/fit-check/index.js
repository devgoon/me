...
module.exports = async function(context, req) {
  const obs = beginRequest(context, req, "fit.check");
  try {
    const apiKey = process.env.ANTHROPIC_API_KEY;
    const databaseUrl = process.env.DATABASE_URL;

    if (!apiKey) {
      context.res = { status: 500, headers: withRequestId({ "Content-Type": "application/json" }, obs.requestId), body: { error: "ANTHROPIC_API_KEY is not configured" } };
      endRequest(context, obs, 500);
      return;
    }

    if (!databaseUrl) {
      context.res = { status: 500, headers: withRequestId({ "Content-Type": "application/json" }, obs.requestId), body: { error: "DATABASE_URL is not configured" } };
      endRequest(context, obs, 500);
      return;
    }
    ...
    if (!jobDescription) {
      context.res = { status: 400, headers: withRequestId({ "Content-Type": "application/json" }, obs.requestId), body: { error: "Request body must include a non-empty 'jobDescription' string" } };
    ...
    try {
      ...
      try {
        ...
        // fallback: return a simple computed result
        parsed = {
          score: 50,
          verdict: "MARGINAL",
    ...
    context.res = { status: 500, headers: withRequestId({ "Content-Type": "application/json" }, obs.requestId), body: { error: error.message || "Fit check failed" } };
  }
};

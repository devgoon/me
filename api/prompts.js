...
const ensureArray = (items) => {
  ...
  if (typeof items === 'object') {
    try {
      return Object.values(items).map((v) => String(v));
    } catch {
      return [];
    }
  }
...
function fitSkillLines(skills) {
  ...
  return skills
    .map((skill) => {
      ...
      const eqs =
        skill.equivalents && Array.isArray(skill.equivalents)
          ? skill.equivalents.filter(Boolean)
          : [];
  ...
  const experiencesText = experiences.length
    ? experiences
        .map((exp) => {
          ...
          return [
            `### ${textOrNA(exp.company_name)} (${dateOrPresent(exp.start_date)} to ${dateOrPresent(
              exp.end_date
            )})`,
  ...
  const educationText =
    payload.education && payload.education.length
      ? payload.education
          .map(
            (ed) =>
              `- ${textOrNA(ed.institution)} — ${textOrNA(ed.degree)}${
                ed.field_of_study ? ' (' + textOrNA(ed.field_of_study) + ')' : ''
              } (${dateOrPresent(ed.start_date)} to ${dateOrPresent(ed.end_date)})${
                ed.grade ? ' — ' + textOrNA(ed.grade) : ''
              }`
          )
          .join('\n')
  ...
  const certificationsText =
    payload.certifications && payload.certifications.length
      ? payload.certifications
          .map(
            (c) =>
              `- ${textOrNA(c.name)} — ${textOrNA(c.issuer)}${
                c.credential_id ? ' (id: ' + textOrNA(c.credential_id) + ')' : ''
              }${c.issue_date ? ' — ' + dateOrPresent(c.issue_date) : ''}${
                c.expiration_date ? ' to ' + dateOrPresent(c.expiration_date) : ''
              }${c.verification_url ? ' — ' + textOrNA(c.verification_url) : ''}`
          )
          .join('\n')
...
function buildChatPrompt(payload) {
  ...
  const parts = [
    `You are an AI assistant representing ${profile.name}, a ${textOrNA(
      profile.title

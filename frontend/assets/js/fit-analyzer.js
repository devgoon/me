...
function educationLinesFromEntries(education, text) {
  ...
  return education
    ...
    .map((e) => {
      ...
      const years =
        (e.startDate || e.start_date ? `${e.startDate || e.start_date || ''}` : '') +
        (e.endDate || e.end_date ? ` - ${e.endDate || e.end_date}` : '');
...
function gapLinesFromEntries(gaps, text) {
  ...
  return gaps
    ...
    .map((entry) => ({
      ...
      interested: Boolean(entry.interested),
    }));
}
...
function analyzeJD(jobDescription, profileStrengths = [], possibleGaps = [], education = []) {
  ...
  let opening =
    "I'd call this a mixed fit: I can likely deliver meaningful value here, but I would want to align expectations on the few areas that are less direct matches.";
  let recommendation =
    'Proceed to a technical conversation focused on the highest-risk requirements, then decide quickly based on concrete gap tolerance.';
  ...
  if (hardGaps > 0) {
    ...
    verdictClass = 'strong';
    opening =
      "I'm a strong fit for this role based on direct overlap in cloud architecture, API delivery, and production reliability outcomes.";
    recommendation =
      'Move forward with a scoped technical interview centered on architecture depth and execution speed in your domain.';
  } else if (score <= 0 || hardGaps >= 3) {
    verdict = 'Probably Not Your Person';
    verdictClass = 'weak';
    opening =
      "I'm probably not your person for this specific role as written, and I'd rather be direct about that than force a weak match.";
    recommendation =
      'If these requirements are truly fixed, I would pass; if they are flexible, reframe around platform/API ownership and we can reassess quickly.';
  }
  ...
  const result = {
    ...
    gaps:
      gapObjs.length > 0
        ? gapObjs.map((g) => (g.interested ? `${g.text} (interested in learning)` : g.text))
        : [
    ...
    transfers:
      transfers.length > 0
        ? transfers.concat(educationMatches)
...
if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
  module.exports = {
    includesAny,
    analyzeJD,
    skillLinesFromEntries,
    gapLinesFromEntries,
    educationLinesFromEntries,
  };
}

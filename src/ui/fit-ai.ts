  ...
  function FitApp() {
    ...
    return e(
      ...
      e(
        ...
        e(
          'button',
          { id: 'analyze-fit', className: 'analyze-btn', type: 'button', onClick: analyze },
          e('i', { className: 'bi bi-stars', 'aria-hidden': 'true' }),
          e('span', null, 'Analyze Fit')
        )
      ),
      result
        ? e(
            'section',
            { className: 'fit-output', 'aria-live': 'polite' },
            e(
              'article',
              { className: 'role-card fit-card' },
              e(
                'div',
                { className: 'fit-verdict-row' },
                e('h2', null, 'Assessment'),
                e('span', { className: `verdict-badge ${result.verdictClass}` }, result.verdict)
              ),
              e('p', { className: 'opening-assessment' }, result.opening),
              e('h3', null, "WHERE I DON'T FIT"),
              e('ul', { className: 'achievement-list' }, result.gaps.map((item) => e('li', { key: `gap-${item}` }, item))),
              e('h3', null, 'WHAT TRANSFERS'),
              e('ul', { className: 'achievement-list' }, result.transfers.map((item) => e('li', { key: `transfer-${item}` }, item))),
              e('h3', null, 'MY RECOMMENDATION'),
              e('p', { className: 'recommendation' }, result.recommendation),
              e(
                'aside',
                { className: 'philosophy-callout' },
                e('strong', null, 'Philosophy:'),
                e(
                  'p',
                  null,
                  'This signals something completely different than "please consider my resume." You\'re qualifying them. Your time is valuable.'
                )
              )
            ),
            e(
              'section',
              { className: 'ask-panel', 'aria-label': 'Ask about this fit assessment' },
              e('label', { className: 'ask-label', htmlFor: 'fit-follow-up' }, 'Ask about this fit assessment'),
              e('textarea', {
                id: 'fit-follow-up',
                className: 'ask-input',
              ...
              e(
                'button',
                { className: 'ask-btn', type: 'button', onClick: askFollowUp },
                'Ask Fit AI'
              ),
              followUpAnswer ? e('div', { className: 'ask-answer' }, followUpAnswer) : null
            )
          )
        : null
    );

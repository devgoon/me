    ...
    try {
      ...
      if (data && typeof data === "object") {
        ...
        if (Array.isArray((data as any).content)) {
          const textBlock = (data as any).content.find((item: any) => item && item.type === "text");
          if (textBlock && typeof textBlock.text === "string") return textBlock.text;
        }
        // Check for data array
        if (Array.isArray((data as any).data)) {
          const textBlock = (data as any).data.find((item: any) => item && item.type === "text");
          if (textBlock && typeof textBlock.text === "string") return textBlock.text;
        }
        // Check for top-level text field
...
module.exports = async function(context, req) {
  const obs = observability.beginRequest(context, req, "chat.ask");
  try {
    ...
    if (!apiKey) {
      ...
      observability.endRequest(context, obs, 500);
      return;
    }
    ...
    if (!userMessage) {
      ...
      observability.endRequest(context, obs, 400);
      return;
    }
    ...
    observability.failRequest(context, obs, error, status);
    context.res = {
      status,
      headers: observability.withRequestId({ "Content-Type": "application/json" }, obs.requestId),
      body: { error: message }
